  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (role !== "artist") {
          await signOut(auth);
          alert(`Access Denied. This account is registered as a Listener.`);
          return;
        }
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (role !== "artist") {
          await signOut(auth);
          alert(`Access Denied. This Google account is already registered as a Listener.`);
          return;
        }
      } else {
        // User is NEW: Create Profile as ARTIST
        await setDoc(docRef, {
          uid: user.uid,
          name: user.displayName,
          email: user.email,
          role: "artist", // Enforce Artist Role
  ...
  return (
    <div className="login-container" style={{ backgroundColor: '#121212', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white' }}>
      <div style={{ backgroundColor: 'black', padding: '40px', borderRadius: '8px', width: '400px', textAlign: 'center', border: '1px solid #333' }}>
        ...
        <h2 style={{ marginBottom: '20px' }}>Log in (Artist)</h2>
        ...
        <p style={{ marginTop: '20px', color: '#b3b3b3', fontSize: '14px' }}>
          Don't have an artist account? <a href="/signup-artist" style={{ color: 'white', fontWeight: 'bold', marginLeft: '5px' }}>Sign up here</a>
        </p>
      </div>
    </div>
  );
